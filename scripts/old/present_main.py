# main.py - Simplified Darwin Chatbot with integrated video and lipsync - UPDATED
...
from simple_final_lipsync import SimplifiedLipSyncSystem  # Using the new lip sync system
...
class DarwinChatbot:
    ...
    def __init__(self):
        ...
        print(f"{Fore.GREEN}[MAIN] Darwin Chatbot initialized with updated lip sync system{Style.RESET_ALL}")

    def initialize_lipsync(self) -> SimplifiedLipSyncSystem:
        """Initialize the updated lip sync system with improved clip configuration"""
        archive_dir = os.path.join(PROJECT_DIR, "archive")
        
        # Updated clip selection odds based on the new system
        # Removed problematic clips and adjusted weights for better performance
        clip_odds = {
            "circle1": 0.0,        # Disabled - marked as bad in new system
            "eye_look1": 0.5,      # Moderate use
            "idle2": 1.0,          # High use - good quality
            "slight_look1": 1.0,   # High use - good quality  
            "slight_shake1": 1.0,  # High use - good quality
            "slight_shake2": 0.1,  # Very low use - marked as problematic
            "nod1": 0.0,           # Disabled - marked as very bad in new system
            "main2": 1.0,          # High use - good quality
        }
        
        return SimplifiedLipSyncSystem(
            archive_directory=archive_dir,
            clip_odds=clip_odds,
            avoid_repeats=True  # Helps create more natural-looking sequences
        )
    ...
    def handle_user_input(self, user_text: str):
        ...
        if self.is_processing:
            # Use JavaScript alert instead of notify to avoid context issues
            nicegui_ui.run_javascript("window.alert('Please wait for the current response to complete');")
            return
    ...
    async def process_response_async(self, user_text: str):
        ...
        # Store chat log reference to avoid context issues
        chat_log_ref = self.chat_log
        ...
        try:
            ...
            if not darwin_response:
                ...
                # Use run_javascript to show error notification to avoid context issues
                nicegui_ui.run_javascript("window.alert('Failed to generate response');")
                return
            ...
            if not os.path.exists(default_voice):
                print(f"{Fore.YELLOW}[MAIN] Default voice not found: {default_voice}{Style.RESET_ALL}")
                ...
                if os.path.exists(voices_dir):
                    ...
                    if voice_files:
                        default_voice = os.path.join(voices_dir, voice_files[0])
                        print(f"{Fore.YELLOW}[MAIN] Using fallback voice: {voice_files[0]}{Style.RESET_ALL}")
            ...
            if not audio_path or not os.path.exists(audio_path):
                ...
                nicegui_ui.run_javascript("window.alert('Audio generation failed');")
                return
            
            print(f"{Fore.GREEN}[MAIN] Audio saved: {audio_path}{Style.RESET_ALL}")
            
            # Step 3: Generate lip-sync video using the updated system
            print(f"{Fore.BLUE}[MAIN] Creating lip-sync video with updated system...{Style.RESET_ALL}")
            ...
            def generate_lipsync():
                try:
                    return self.lipsync_system.generate_lip_sync_video(
                        ...
                        use_sequential=True,
                        target_clips=None  # auto-calculate based on audio duration
                    )
                except Exception as e:
                    print(f"{Fore.RED}[MAIN] Lipsync generation error: {e}{Style.RESET_ALL}")
                    return None
            ...
            if not lipsync_video or not os.path.exists(lipsync_video):
                ...
                nicegui_ui.run_javascript("window.alert('Lip-sync video generation failed');")
                return
            ...
            # Clean up old files periodically (keep more files for better performance)
            self.video_manager.cleanup_old_lipsync_videos(keep_last=10)
            
            # Clean up old audio files too
            self.cleanup_old_audio_files(audio_path, keep_last=5)
            
        except Exception as e:
            print(f"{Fore.RED}[MAIN] Error processing response: {e}{Style.RESET_ALL}")
            import traceback
            traceback.print_exc()
            # Use JavaScript alert to avoid UI context issues
            error_msg = str(e).replace("'", "\\'")  # Escape quotes for JavaScript
            nicegui_ui.run_javascript(f"window.alert('Processing error: {error_msg}');")
        
        finally:
            self.is_processing = False

    def cleanup_old_audio_files(self, current_audio_path: str, keep_last: int = 5):
        """Clean up old audio files to save disk space"""
        try:
            audio_dir = os.path.dirname(current_audio_path)
            ...
            for file_path, _ in audio_files[keep_last:]:
                try:
                    os.remove(file_path)
                    print(f"{Fore.BLUE}[MAIN] Cleaned up old audio file: {os.path.basename(file_path)}{Style.RESET_ALL}")
                except Exception as e:
                    print(f"{Fore.YELLOW}[MAIN] Could not remove audio file {file_path}: {e}{Style.RESET_ALL}")
                    
        except Exception as e:
            print(f"{Fore.YELLOW}[MAIN] Audio cleanup error: {e}{Style.RESET_ALL}")
    ...
    def handle_voice_change(self, voice_name: str):
        ...
        try:
            ...
            if not os.path.exists(voice_path):
                print(f"{Fore.RED}[MAIN] Voice file not found: {voice_path}{Style.RESET_ALL}")
                nicegui_ui.notify(f"Voice file not found: {voice_name}", type="negative")
                return
            ...
            nicegui_ui.notify(f"Voice changed to: {voice_name}", type="positive")
            
        except Exception as e:
            print(f"{Fore.RED}[MAIN] Error changing voice: {e}{Style.RESET_ALL}")
            nicegui_ui.notify(f"Error changing voice: {str(e)}", type="negative")

    def run(self):
        """Run the chatbot application"""
        print(f"{Fore.GREEN}{'='*60}{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}Darwin Chatbot - Updated with Enhanced Lip Sync{Style.RESET_ALL}")
        print(f"{Fore.GREEN}{'='*60}{Style.RESET_ALL}")
