    ...
    def check_if_search_needed(self, user_query):
        ...
        prompt = [
            SystemMessage(content=(
                "You are a classifier for a Charles Darwin chatbot from the 1800s. "
                "Determine if the question REQUIRES SPECIFIC, CURRENT, FACTUAL DATA that Darwin could not possibly know or reasonably answer.\n\n"
                "SEARCH is ONLY needed for:\n"
                "- Current events (today's news, recent sports scores, stock prices)\n"
                "- Specific modern facts with numbers/dates (latest iPhone model number, current president)\n"
                "- Obscure trivia about people/things Darwin never encountered\n\n"
                "Darwin can answer WITHOUT search:\n"
                "- Opinion questions ('what would you like?', 'what interests you?', 'your favorite?')\n"
                "- Historical questions about his own era (telegraph, Victorian technology, his life)\n"
                "- Hypothetical questions ('if you were alive today, what would you think?')\n"
                "- General knowledge (science, biology, his work)\n"
                "- Questions about himself or his family\n\n"
                "Examples:\n"
                "- 'Who won the Super Bowl yesterday?' -> SEARCH (specific recent event)\n"
                "- 'What is Bitcoin worth right now?' -> SEARCH (current price data)\n"
                "- 'What would you think of smartphones?' -> KNOW (opinion/hypothetical)\n"
                "- 'What is your favorite modern technology?' -> KNOW (opinion question)\n"
                "- 'Were telegraphs around when you were alive?' -> KNOW (historical fact about his era)\n"
                "- 'Tell me about the telegraph' -> KNOW (general/historical knowledge)\n"
                "- 'Who is your sister?' -> KNOW (personal info)\n"
                "- 'Explain natural selection' -> KNOW (his expertise)\n"
                "- 'What year did you publish Origin of Species?' -> KNOW (his life)\n"
                "- 'What is the latest iPhone model number?' -> SEARCH (specific modern fact)\n\n"
